...
- compute the sum of all elements in the array once to derive the right sum efficiently for an 
index
- leftSum starts at 0 so that I can iterate through the array and update leftSum
 by adding the current index
 - for each index, the right sum is calculated by subtracting the left sum and the current index
 from the totalSum
 - once left and right are equal, i can return the index (if there is one, otherwise,
 I return a -1)
